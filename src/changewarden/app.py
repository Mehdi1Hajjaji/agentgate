...
class ChangeWardenApp:
    ...
    def decide(self, request_id: str, approver: str, approval_reason: str, approve: bool) -> tuple[dict, dict | None, int | None]:
        """Record an independent decision and optionally dispatch server-side.

        Approval is bound to the stored typed request.  If policy opts into
        automatic execution, the Gate reloads policy and crosses the GitHub
        boundary itself; the Agent never receives a second execution power.
        """
        request = self.store.get_request(request_id)
        if request is None:
            raise LookupError("request_not_found")
        self.policy = Policy.load(self.settings.policy_path)
        current = self.policy.evaluate(request["repository"], request["action"], request["payload"])
        if approve and not current.allowed:
            item = self.store.decide(request_id, approver, f"Policy changed before decision: {current.reason}", False)
            return self.public_request(item), {"allowed": False, "reason": "policy_changed_before_decision"}, 403
        item = self.store.decide(request_id, approver, approval_reason, approve)
        if approve and bool(current.constraints.get("auto_execute_on_approval", False)):
            execution, status = self.execute(request_id)
            refreshed = self.store.get_request(request_id) or item
            return self.public_request(refreshed), execution, status
        return self.public_request(item), None, None
...
def form_page(request: dict | None, error: str = "") -> bytes:
    if request is None:
        ...
        content = f"""<main><p class='eyebrow'>CHANGEWARDEN · HUMAN DECISION</p><h1>{title}</h1><p class='status'>{status}</p><dl><dt>Requested by</dt><dd>{escape(request['requester_id'])}</dd><dt>Why</dt><dd>{escape(request['request_reason'])}</dd></dl><h2>Bounded action payload</h2><pre>{payload}</pre>"""
...
def handler_factory(app: ChangeWardenApp):
    class Handler(BaseHTTPRequestHandler):
        ...
        def _agent(self) -> str | None:
            ...
            actor=self.headers.get("X-ChangeWarden-Actor","").strip()
            return actor if 1 <= len(actor) <= 120 else None

        def _admin(self) -> bool:
            given=self.headers.get("X-ChangeWarden-Admin-Key","").strip()
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if path.startswith("/v1/requests/") and path.endswith("/decision"):
                    if not self._admin(): return self._json(401,{"error":"admin_auth_required"})
                    request_id=path.split("/")[3]; data=self._body_json(); item, execution, status=app.decide(request_id,str(data.get("approver_id","")),str(data.get("reason","")),str(data.get("decision",""))=="approve"); return self._json(status or 200,{"request":item,"execution":execution})
                ...
                if path.startswith("/requests/") and path.endswith("/decision"):
                    ...
                    item, _, _=app.decide(request_id,form.get("approver_id",[""])[0],form.get("reason",[""])[0],form.get("decision",[""])[0]=="approve")
...
def main() -> None:
    ...
    if not settings.agent_token or not settings.admin_token:
        raise SystemExit("Set CHANGEWARDEN_AGENT_TOKEN and CHANGEWARDEN_ADMIN_TOKEN; see .env.example")
    app=ChangeWardenApp(settings)
    server=ThreadingHTTPServer((os.environ.get("CHANGEWARDEN_HOST","127.0.0.1"),int(os.environ.get("CHANGEWARDEN_PORT","8080"))),handler_factory(app))
    print(f"ChangeWarden listening on http://{server.server_address[0]}:{server.server_address[1]}")
